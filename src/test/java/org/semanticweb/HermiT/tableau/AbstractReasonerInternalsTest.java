...
public abstract class AbstractReasonerInternalsTest extends AbstractReasonerTest {
    ...
    protected DLOntology getDLOntology(Configuration c, Set<DescriptionGraph> dgs) {
        ...
        return (DLOntology) clausification.preprocessAndClausify(m_ontology, dgs == null?Collections.emptySet():dgs)[1];
    }
    ...
    protected Tableau getTableau(Set<DescriptionGraph> dgs) {
        ...
        return new Tableau(new InterruptFlag(-1), getTableauMonitor(), expansionStrategy, false, dlOntology, null,
    ...
    protected static void assertRetrieval(ExtensionTable extensionTable, Object[] searchTuple,
            ExtensionTable.View extensionView, Object[][] expectedTuples) {
    ...
    protected static void assertLabel(Tableau tableau, Node node, Concept... expected) {
        Set<Concept> actual = new HashSet<>();
        ExtensionTable.Retrieval retrieval = tableau.getExtensionManager().getBinaryExtensionTable()
                .createRetrieval(new boolean[] { false, true }, ExtensionTable.View.TOTAL);
    ...
    protected static DLOntology getTestDLOntology(Set<DLClause> dlClauses) {
        ...
        return new DLOntology("opaque:test", // ontology_URI
                dlClauses, // clauses
